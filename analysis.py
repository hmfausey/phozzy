...
def input_output_density_plot(
    num, walkers, filter_edges, save_string, highz_cutoff, acc, Ebv_fitting=True
):
    ...
    # and output redshift)
    if Ebv_fitting:
        _, _, _, _, z_in, z_out, _, _ = import_results(
            num, walkers, save_string, Ebv_fitting=Ebv_fitting
        )

    else:
        _, _, _, _, z_in, z_out = import_results(
            num, walkers, save_string, Ebv_fitting=Ebv_fitting
        )
    ...
    plt.hist2d(
        z_in, z_out, bins=(50, 50), range=[[0, z_range], [0, z_range]], cmap=plt.cm.jet
    )
    plt.colorbar()
    ...
    for i in range(len(associated_redshift)):
        plt.plot(
            [associated_redshift[i], associated_redshift[i]], [0, z_range + 1], "w--"
        )
    # Show high vs low redshift cutoff
    plt.plot(
        [0, z_range + 1],
        [highz_cutoff, highz_cutoff],
        "w-",
        [highz_cutoff, highz_cutoff],
        [0, z_range + 1],
        "w-",
