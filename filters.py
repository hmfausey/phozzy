...
def filter_observations(lam_obs, spectrum, filter_edges, noisy = False, uncertainty = 0, sig_noise = 3):
    ...
    for i in range(len(lam_obs)-1):
        if filter_num == (len(filter_edges)):
            ...
            filter_vals[filter_num] += np.abs(0.5*(0.5*(spectrum[i] + spectrum[i+1])*(lam_obs[i+1] - lam_obs[i])))
        
        elif (filter_edges[filter_num][0] <= lam_obs[i]) and (lam_obs[i+1] <= filter_edges[filter_num][-1]):
            #If the wavelength range being integrated is fully in the current
             #filter, full flux contribution is added to that filter
            filter_vals[filter_num] += np.abs(0.5*(spectrum[i] + spectrum[i+1])*(lam_obs[i+1] - lam_obs[i]))
            ...
            if (filter_num) < (len(filter_edges)-1):
                ...
                filter_vals[filter_num+1] += np.abs(0.5*(0.5*(spectrum[i] + spectrum[i+1])*(lam_obs[i+1] - lam_obs[i])))
            ...
            filter_vals[filter_num+1] += np.abs(0.5*(spectrum[i] + spectrum[i+1])*(lam_obs[i+1] - lam_obs[i]))
