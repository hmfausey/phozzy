...
def phozzy(
    ...
    make_data.build_set(
        ...
        Ebv_input=Ebv_input,
        Ebv_prior=Ebv_prior,
        Ebv_fitting=Ebv_fitting,
        flux_input=flux_input,
        upper_limit=Ebv_upper_limit,
    )

    # Load the data back in
    GRB_params = np.loadtxt("GRB_parameters_" + save_string + ".txt", delimiter=" ")
    filter_obs = np.loadtxt(
        "Filter_observations_" + save_string + ".txt", delimiter=" "
    )
    uncertainties = np.loadtxt("Uncertainties_" + save_string + ".txt", delimiter=" ")
    initial_guesses = np.loadtxt(
        "Initial_guesses_" + save_string + ".txt", delimiter=" "
    )
    ...
    for i in range(num):
        ...
        mcmc.mcmc(
            ...
            result_string,
            filter_edges,
            overWrite_initial_guess=overWrite_initial_guess,
            nwalkers=nwalkers,
            burnin=burnin,
            produc=produc,
            extinction_law=extinction_law,
    ...
    # Run the analysis
    analysis.input_output_density_plot(
        num,
        nwalkers,
        filter_edges,
        analysis_string,
