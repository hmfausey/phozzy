...
from miralda_escude.construct_spectrum import Teff_total
...
def build(filter_edges, f_0, beta, z, E_bv, logNH, extinction_law="smc"):
    ...
    lam_obs = lam_emit*(1+z) * 10**(-4)
    
    #print(lam_obs)
    Teff = Teff_total(logNH, 0, z, lam_obs, z, z, 5.5)
    #print(Teff)
    
    Teff[np.isnan(Teff)] = np.inf
    
    spectrum = dinosaur*np.exp(-Teff)
    
    return lam_obs*10**(4), spectrum
