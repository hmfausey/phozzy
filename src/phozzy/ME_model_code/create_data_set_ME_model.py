...
# -*- coding: utf-8 -*-
"""
Created on Fri Oct 18 14:34:36 2024
...
import build_spectrum_ME_model

import filters

import d_L_calc as dcal

#filter_centers = [0.57,0.75,1.06,1.45,2.05]
...
def build_set(num, z_l, z_u, x_H1, filter_edges, save_string, extinction_law = 'smc', uncertainty = 0.05, sig_noise = 3, z_input = 'uniform', z_prior = 'uniform', Ebv_input = 'evolving', Ebv_prior = 'evolving', Ebv_fitting = True, flux_input = 'kann', upper_limit = 0):
    ...
    if Ebv_fitting:
        ...
        initial_guesses = np.zeros((num, 5))
        GRB_params = np.zeros((num,5))
   ...
   #For flux distribution
    ...
    while i < num:
        ...
        # Get values for logNH
        logNHval = random.uniform(19,23)
        logNHguess = random.uniform(19,23)
        ...
        if z_input == 'uniform':
            ...
            raise Exception("Invalid redshift input distribution. Choices are 'uniform' or 'expected' and must be a string")
        ...
        if Ebv_fitting:
            ...
            OG_params = np.array([Fval, betaval, zval, Ebvval, logNHval])
            #Build the spectrum based on the GRB parameters
            lam_obs, spectrum = build_spectrum_ME_model.build(filter_edges, Fval, betaval, zval, Ebvval,logNHval, z_l, z_u, x_H1, extinction_law=extinction_law)
            ...
            plt.plot(lam_obs, spectrum)
            plt.scatter([0.57,0.75,1.06,1.45,2.05], filter_vals)
            plt.show()
            plt.close()
            ...
            if filter_vals[-1] >= 5*sig_noise:
                ...
                guess_array = np.array([Fguess, betaguess, zguess, Ebvguess, logNHguess])
            ...
            OG_params = np.array([Fval, betaval, zval, logNHval])
            
            filter_centers = filters.get_filter_centers(filter_edges)
            lam_obs, spectrum = build_spectrum_ME_model.build(filter_edges, Fval, betaval, zval, 0, logNHval, z_l, z_u, x_H1)
            _, filter_vals, quadrature = filters.filter_observations(lam_obs, spectrum, filter_edges, noisy=True, uncertainty=uncertainty, sig_noise=sig_noise)
            ...
            if filter_vals[-1] >= 5*sig_noise:
                ...
                guess_array = np.array([Fguess, betaguess, zguess, logNHguess])
