...
##put in values from original simulation by adding equal signs in functions

def import_results(num, walkers, test_run, Ebv_fitting = True): 
    ...
    for i in range(num):
        #Using savestring to identify the correct set of runs, and i to pull
         ...
         #parameter array
        filestring = test_run + "/"+test_run+"_"+"results_"+str(i)+"_datastore.txt"
...
def input_output_density_plot(num, walkers, filter_edges, save_string, highz_cutoff, acc, Ebv_fitting=True):
    ...
    pure = 0
    accurate = 0
    for i in range(len(z_out)): #change z_in and z_out starting here
