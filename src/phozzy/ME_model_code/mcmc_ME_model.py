...
# -*- coding: utf-8 -*-
"""
Created on Fri Oct 18 14:44:57 2024

@author: michaelaschulman
"""
...
from phozzy_main import build_spectrum_ME_model

from phozzy_main import filters
...
def mcmc(x, y, yerr, initial_guess, GRB_params, save_string, filter_edges, z_l, z_u, x_H1, nwalkers=50, burnin=250, produc=500, extinction_law = 'smc', z_prior = 'uniform', Ebv_prior = 'evolving', Ebv_fitting = True, upper_limit = 0, parallel = False, cpu_num = int(3/4*os.cpu_count())):
   ##Performs a Markov-Chain Monte-Carlo fitting method for a set of simulated
    ...
    if Ebv_fitting:
        #If Ebv is a fitting parameter, include it for parameter names and
         #labels
        param_names = ["F0", "beta", "z", "Eb-v", "logNH"]
        labels = [r'$F_0$', r'$\beta$', r'z', r'$E_{b-v}$', r'logNH']
        f_real, beta_real, z_real, Ebv_real, logNH_real = GRB_params
    else:
        #If Ebv is not a fitting parameter, exclude it from parameter names and
         #labels
        param_names = ["F0", "beta", "z", 'logNH']
        labels = [r'$F_0$', r'$\beta$', r'z', r'logNH' ]
        f_real, beta_real, z_real, logNH_real = GRB_params
        Ebv_real = 0
    ...
    if parallel:
        with mp.Pool(processes = cpu_num) as pool:
            #set up sampler
            sampler = emcee.EnsembleSampler(nwalkers, ndim, lnprob, args = (y, yerr, filter_edges, z_prior, Ebv_prior, Ebv_fitting, upper_limit, z_l, z_u, x_H1, extinction_law), pool = pool)
        ...
        sampler = emcee.EnsembleSampler(nwalkers, ndim, lnprob, args = (y, yerr, filter_edges, z_prior, Ebv_prior, Ebv_fitting, upper_limit, z_l, z_u, x_H1, extinction_law))
    ...
    lam_obs, spectrum = build_spectrum_ME_model.build(filter_edges, f_real, beta_real, z_real, Ebv_real, logNH_real, z_l, z_u, x_H1, extinction_law=extinction_law)
    ...
    for i in range(nwalkers):
        ...
        if Ebv_fitting:
            Fnew, betanew, znew, E_bvnew, logNH_new = s
        else:
            Fnew, betanew, znew, logNH_new = s
            E_bvnew = 0
        ...
        #Finding the spectrum associated with the final positions of each 
         ...
         #with it
        lam_obs, spectrum = build_spectrum_ME_model.build(filter_edges, Fnew, betanew, znew, E_bvnew, logNH_new, z_l, z_u, x_H1, extinction_law=extinction_law)
        filter_vals, _, _ = filters.filter_observations(lam_obs, spectrum, filter_edges)
        
        #Save results
        if Ebv_fitting:
            final_data = np.array([f_real, Fnew, beta_real, betanew, z_real, znew, Ebv_real, E_bvnew,logNH_real, logNH_new])
        else:
            final_data = np.array([f_real, Fnew, beta_real, betanew, z_real, znew, logNH_real, logNH_new])
...
def model(params, filter_edges, Ebv_fitting, z_l, z_u, x_H1, extinction_law):
    ...
    if Ebv_fitting:
        ...
        f_0, beta, z, E_bv, logNH = params
        lam_obs, spectrum = build_spectrum_ME_model.build(filter_edges, f_0, beta, z, E_bv, logNH, z_l, z_u, x_H1, extinction_law=extinction_law)
        filter_vals, _, _ = filters.filter_observations(lam_obs, spectrum, filter_edges)
    else:
        #If not, set E_{B-V} to 0 and determine result using other parameter 
         #values
        f_0, beta, z = params
        lam_obs, spectrum = build_spectrum_ME_model.build(filter_edges, f_0, beta, z,0, logNH, z_l, z_u, x_H1)
        filter_vals, _, _ = filters.filter_observations(lam_obs, spectrum, filter_edges)
        #Return model filter values
    return filter_vals

def chi_squared(params, y, yerr, filter_edges, Ebv_fitting, z_l, z_u, x_H1, extinction_law):
    ...
    model_filters = model(params, filter_edges, Ebv_fitting, z_l, z_u, x_H1, extinction_law)
...
def lnlikelihood(params, y, yerr, filter_edges, Ebv_fitting, z_l, z_u, x_H1, extinction_law):
    ...
    chi2 = chi_squared(params, y, yerr, filter_edges, Ebv_fitting, z_l, z_u, x_H1, extinction_law)
...
def lnprior(params, z_prior, Ebv_prior, Ebv_fitting, upper_limit):
    ...
    if Ebv_fitting:
        ...
        f_0, beta, z, logNH = params
        Ebv_prior = 1.0
    
    #Make sure other parameters have valid values
    if (f_0 > 0) and (0 < beta <= 2.5) and (0 < z < 25) and (18 <logNH< 24):
...
def lnprob(params, y, yerr, filter_edges, z_prior, Ebv_prior, Ebv_fitting, upper_limit, z_l, z_u, x_H1, extinction_law):
    ...
    if np.isfinite(prior):
        prob = prior + lnlikelihood(params, y, yerr, filter_edges, Ebv_fitting,z_l, z_u, x_H1, extinction_law)
