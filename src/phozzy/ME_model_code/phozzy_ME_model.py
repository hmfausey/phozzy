...
# -*- coding: utf-8 -*-
"""
Created on Fri Oct 18 14:40:09 2024
...
import create_data_set_ME_model as make_data

import mcmc_ME_model
...
def phozzy(num, x_H1, z_u, z_l, filter_edges, save_string, extinction_law = 'smc', uncertainty = 0.05, sig_noise = 3, nwalkers = 50, burnin = 250, produc=500, z_input = 'uniform', z_prior = 'uniform', Ebv_input = 'evolving', Ebv_prior = 'evolving', Ebv_fitting = True, Ebv_upper_limit = 0, flux_input = 'kann', parallel = False, cpu_num = int(3/4*os.cpu_count()), highz_threshold = 5, acc = 0.1):
    ...
    #Create a set of GRB spectra and determine their associated photometric 
     ...
     #working directory
    make_data.build_set(num, z_l, z_u, x_H1, filter_edges, save_string, extinction_law = extinction_law, uncertainty = uncertainty, sig_noise = sig_noise, z_input = z_input, z_prior = z_prior, Ebv_input = Ebv_input, Ebv_prior = Ebv_prior, Ebv_fitting = Ebv_fitting, flux_input=flux_input, upper_limit=Ebv_upper_limit)
    ...
    for i in range(num):
        ...
        mcmc_ME_model.mcmc(x, y, yerr, initial_guess, GRB_param_arr, result_string, filter_edges, z_l, z_u, x_H1, nwalkers=nwalkers, burnin=burnin, produc=produc, extinction_law = extinction_law, z_prior = z_prior, Ebv_prior = Ebv_prior, Ebv_fitting = Ebv_fitting, upper_limit=Ebv_upper_limit, parallel = parallel, cpu_num = cpu_num)
