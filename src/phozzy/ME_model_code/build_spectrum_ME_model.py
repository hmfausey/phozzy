...
def build(filter_edges, f_0, beta, z, E_bv, logNH, z_l, z_u, x_H1, extinction_law = 'smc'):
    ...
    #convert to cm then back to microns
    lam_obs = lam_obs*(10**(-4))
    #Pull ME Model for Optical Depth
    T_eff = optical_depth_ME.Teff_total(lam_obs,logNH, x_H1, z_l, z,z, z_u)
    
    #T_eff = optical_depth_ME.Teff_total(lam_emit,logNH, x_H1, z_l, z,z, z_u)
    lam_obs = lam_obs*(10**4)
    
   # Calculate the observed wavelength threshold for redshifted cutoff
    threshold_obs = ly_alph * (1 + z)
    
    T_eff = np.array(T_eff, dtype = np.float64)
    
    #print(np.exp(-T_eff))
   
    F_rec = dinosaur * np.exp(-T_eff)
    
    F_rec[lam_obs < threshold_obs] = 0  # Directly set values below threshold to zero
    
            
    
    return lam_obs, F_rec
